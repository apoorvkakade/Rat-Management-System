...
def home_function(request):
    if request.user.is_authenticated:
        if request.method=='GET':
            ...
            #     # quarantine_display_obj = Quarantine.objects.latest('entry_date')
            #     # breeding_display_obj = Breeding.objects.latest('entry_date')
            #     # pregnant_display_obj = Pregnant.objects.latest('entry_date')
            #     # weaning_display_obj = Weaning.objects.latest('entry_date')
            #     # young_display_obj=Young.objects.latest('entry_date')
            #     # exp_display_obj=Experimental.objects.latest('entry_date')
            #     # gtotal_display_obj=Grand_total.objects.latest('entry_date')
            #
            # else:
            #     display_object.set_values(q_m=0,q_f=0,b_m=0,b_f=0,p=0,w=0,
            #                           m=0,e_m=0,e_f=0,t_m=0,t_f=0,y_m=0,y_f=


            else:
                print("Elseoieoi")
                # for specie in species:
                #     entry_display_obj=Display()
                #     display_objects.append(entry_display_obj)
